...
cat ./galaxy.yml | shyaml key-values-0 dependencies |
while read-0 key value; do
    fval=$(echo "${value}" | tr -d '=')
    sval=$(cat ./requirements.yml | shyaml get-value collections | grep -A1 ${key} | grep -v ${key} | cut -d ':' -f 2 | tr -d ' ')
    if [ "$fval" = "$sval" ]; then
        echo "Versions for ${key} are the same in requirements.yml and galaxy.yml"
    else
        echo "For the ${key} dependency there is a mismatch in"
        echo "/kubeinit/ansible/requirements.yml and /kubeinit/ansible/galaxy.yml"
